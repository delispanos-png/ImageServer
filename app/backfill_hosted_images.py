...
async def process_record(record: dict, images_dir: Path, semaphore: asyncio.Semaphore) -> str:
...
async def run_backfill(limit: int, concurrency: int, barcode_filter: str, input_file: str, resume: bool, state_file: str) -> None:
    ...
    try:
        ...
        for local_index, record in enumerate(source_iterable, start=1):
            ...
            task = asyncio.create_task(process_record(record, images_dir, semaphore))
            pending.add(task)
            scheduled += 1
