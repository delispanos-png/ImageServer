...
def main() -> None:
    ...
    for offset in range(0, len(barcodes), max(1, args.batch_size)):
        ...
        existing_docs = {
            str(doc.get("Barcode", "")).strip(): doc
            for doc in db.products.find({"Barcode": {"$in": chunk}})
        }
        operations = []
        for barcode in chunk:
            existing = existing_docs.get(barcode)
            if not existing:
                continue
            row = lookup[barcode]
            updates = {
            ...
            updates.update(
                build_catalog_quality_updates(
                    candidate,
                    evaluator="automation:update_categories_from_excel",
                )
            )
            operations.append(
                UpdateOne(
